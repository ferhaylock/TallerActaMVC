...
<html>
	...
	<body>
	<form class="form" action="./Vistas/router.php?controller=Login&action=login" method="POST" autocomplete="off">
	<?php
		 isset($vali) ? $vali->resultBlock($errors) : '';
		?>

	<h2 class="form__title">Inicia Sesión</h2>
        <p class="form__paragraph">Crea tu cuenta aqui: <a href="./Vistas/router.php?controller=Registro&action=validaciones" class="form__link">Entra aquí</a></p>
        ...
        <?php echo isset($_GET['pass'])?  "<div style='color:red' class='alert alert-success' role='alert'>         La contraseña es incorrecta
            </div> " : ''?>
